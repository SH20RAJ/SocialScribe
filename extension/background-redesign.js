// SocialScribe+ Redesigned Background Script
...
class SocialScribeBackground {
  ...
  handleFirstInstall() {
    ...
    this.showNotification({
      ...
      title: 'Welcome to SocialScribe+!',
      message: 'Your AI writing assistant is ready. Try Ctrl+Shift+S on any text field!'
    });
  ...
  handleUpdate(previousVersion) {
    ...
    this.showNotification({
      type: 'basic',
      iconUrl: 'icons/icon48.png',
      title: 'SocialScribe+ Updated!',
      message: 'New features: Smart suggestions, better UI, and more tones!'
    });
  }
  ...
  async handleMessage(request, sender, sendResponse) {
    try {
      switch (request.action) {
        ...
        case 'getHistory':
          const history = await this.getStorageData('socialscribeHistory');
  ...
  async addToHistory(entry) {
    let history = await this.getStorageData('socialscribeHistory') || [];
    ...
    await this.setStorageData('socialscribeHistory', history);
  }
  ...
  trackEvent(event, data = {}) {
    ...
    // Could send to privacy-focused analytics service
